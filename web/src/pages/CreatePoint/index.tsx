...
// array ou objeto: manualmente informar informar o tipo da variável
interface Item {
	id: number,
	title: string,
	image_url: string
...
const CreatePoint = () => {
  ...
  const history = useHistory() // permite ir de um componente a outro de um botão

	/* useEffect()
	 * Executa uma função a partir de um parâmetro
	 * Deixando um array vazio ele executa apenas uma única vez
	 * Colocando uma variável dentro do array ele executa quando a variável mudar
   */
  ...
  useEffect(() => {
	...
	useEffect(() => {
		api.get('items').then((resp: AxiosResp) => {
			setItems(resp.data)
	  })
	}, [])
	...
	useEffect(() => {
    if (selectedUf === '0') return // retorna caso seja o valor padrão
  ...
  async function handleSubmit(event: FormEvent) {
    event.preventDefault() // não envia o usuário para outra rota depois do submit
